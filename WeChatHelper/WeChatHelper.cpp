...
// 返 回 值: void
//***********************************************************
void CWeChatHelperApp::RunSingle()
{
    HANDLE hMutex = NULL;
    hMutex = CreateMutexA(NULL, FALSE, "Greatfar");
    if (hMutex)
    {
        if (GetLastError() == ERROR_ALREADY_EXISTS)
        {
            // 向已存在的实例发送 显示窗口消息
            CWnd* pWnd = CWnd::FindWindow(NULL, L"微信助手");
            if (pWnd == NULL) {
                MessageBoxA(NULL, "无法找到微信助手", "tips", 0);
            }
            COPYDATASTRUCT GetInformation;
            GetInformation.dwData = WM_DesktopShowWindow;
